...
from webapp.forms import LoginForm, RegistrationForm, UpdateAccountForm, PostForm
...
def save_picture(form_picture):
    ...
    picture_path = os.path.join(app.root_path, 'static/profile_pics' , picture_fn) #everything concatenated in a single path
    output_size = (125, 125)
    i = Image.open(form_picture)
    i.thumbnail(output_size)
    i.save(picture_path) #resize large images to a small size
    return picture_fn
...
def save_picture_post(form_picture):
    ...
    _, f_ext = os.path.splitext(form_picture.filename)
    picture_fn = random_hex + f_ext
    picture_path = os.path.join(app.root_path, 'static/post_pics' , picture_fn) #everything concatenated in a single path
    output_size = (1200, 680 )
    i = Image.open(form_picture)
    i.thumbnail(output_size)
    i.save(picture_path) #resize large images to a small size
    return picture_fn   
...
def new_post():
    form = PostForm()
    if form.validate_on_submit():
        post = Post(title = form.title.data, content = form.content.data, post_image_file = form.picture_file.data, author = current_user)
